...
Xs <- data.frame(V1=rnorm(n), V2=rnorm(n))

# biased reference matrix
# = subset of the possibles + oversampling in some regions of the state space
Xb <- rbind(Xs, filter(data.frame(V1=rnorm(n), V2=rnorm(n)), V1>0, V2<2.5, V2>-2.5))
n <- 1000
...
Y$ps  <- nppen(Xs, Y[,1:2])
Y$psr <- nppen(Xsr[,1:2], Y[,1:2])
Y$pb  <- nppen(Xb, Y[,1:2])
Y$pbr <- nppen(Xbr[,1:2], Y[,1:2])

Yt <- gather(Y, "source", "proba", starts_with("p"))
